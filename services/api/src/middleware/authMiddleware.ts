...
export interface AuthMiddlewareOptions {
  required?: boolean; // Se true, rejeita requisições sem token válido
  allowedServices?: string[]; // Lista de serviços permitidos
  requiredScopes?: string[]; // Scopes obrigatórios
  skipPaths?: string[]; // Caminhos que não precisam de autenticação
}
...
export class AuthMiddleware {
  ...
  constructor(
    ...
    this.logger = new LoggingService('auth-middleware');
  }
      ...
      try {
        ...
        if (this.options.requiredScopes && validationResult.payload?.scope) {
          const hasRequiredScopes = this.options.requiredScopes.every(scope => 
            validationResult.payload!.scope!.includes(scope)
          );
   ...
   */
  authenticateWebhook(secretKey: string) {
   ...
   */
  requireService(allowedServices: string[]) {
    return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      if (!req.user) {
        return this.handleAuthError(res, 'Usuário não autenticado');
      }
  ...
  private handleAuthError(res: Response, message: string): void {
