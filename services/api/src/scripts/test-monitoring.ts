...
class TestMonitor {
  ...
  constructor() {
    ...
    this.logFile = join(process.cwd(), 'logs', `test-execution-${timestamp}.log`);
    this.metricsFile = join(process.cwd(), 'logs', `test-metrics-${timestamp}.json`);
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.auditService = new AuditService(new LoggingService());
  ...
  async recordTestStart(suiteName: string, testName: string): Promise<string> {
  ...
  async recordTestEnd(
    ...
    const metric = this.metrics.find(m => 
      `${m.testSuite}-${m.testName}-${m.startTime.getTime()}` === testId
    );
    ...
    await this.log(logLevel, 
      `Teste finalizado: ${metric.testSuite} > ${metric.testName} - ${status} (${metric.duration}ms)`
    );
  ...
  async generateReport(suiteName?: string): Promise<TestSuiteReport[]> {
    ...
    for (const suite of suites) {
      ...
      if (suiteMetrics.length === 0) continue;
  ...
  async saveReport(reports: TestSuiteReport[]): Promise<void> {
    try {
      const reportData = {
        reports,
      ...
      await fs.writeFile(this.metricsFile, JSON.stringify(reportData, null, 2));
...
let globalMonitor: TestMonitor | null = null;

export function getTestMonitor(): TestMonitor {
  if (!globalMonitor) {
...
export async function startTestMonitoring(): Promise<TestMonitor> {
...
export async function recordTest(
  ...
  try {
    ...
    await monitor.recordTestEnd(testId, 'failed', (error as Error).message || String(error));
    throw error;
  }
}
