...
exports.callbackHandler = exports.callbacksApi = void 0;
...
const webhookValidationMiddleware_1 = require("../middleware/webhookValidationMiddleware");
...
const authService = new AuthenticationService_1.AuthenticationService(authConfig.googleCloud, authConfig.jwt);
// Inicializar serviços
const db = admin.firestore();
const logger = new LoggingService_1.LoggingService();
const metricsService = new MetricsService_1.MetricsService('callbacks-api');
const cloudRunClient = new CloudRunClient_1.CloudRunClient(process.env.CLOUD_RUN_SERVICE_URL || 'https://analysis-service-url', {
    projectId: process.env.GOOGLE_CLOUD_PROJECT,
    serviceAccountEmail: process.env.CLOUD_RUN_SERVICE_ACCOUNT,
    audience: process.env.CLOUD_RUN_SERVICE_URL
});
const parameterEngine = new ParameterEngine_1.ParameterEngine(db);
const orchestrator = new AnalysisOrchestrator_1.AnalysisOrchestrator(db, process.env.CLOUD_RUN_SERVICE_URL || 'https://analysis-service-url', process.env.GOOGLE_CLOUD_PROJECT || 'default-project');
const notificationService = new NotificationService_1.NotificationService(process.env.GOOGLE_CLOUD_PROJECT || 'default-project');
const auditService = new AuditService_1.AuditService(logger);
// Inicializar CallbackHandler
const callbackHandler = new CallbackHandler_1.CallbackHandler(orchestrator, notificationService, metricsService, auditService, callbackSecurityConfig);
...
const webhookValidation = (0, webhookValidationMiddleware_1.createWebhookValidation)({
    ...
    customErrorHandler: (error, req, res) => {
        ...
        res.status(401).json((0, utils_1.createErrorResponse)(error.code || 'WEBHOOK_VALIDATION_FAILED', error.error || 'Falha na validação do webhook', error.details, req.requestId));
    }
});
...
const securityManager = (0, security_1.initializeSecurity)(db, logger, metricsService, {
    rateLimit: {
 ...
 */
app.post('/callback/analysis', webhookValidation.validate(), webhookValidation.rateLimit(100, 60000), // 100 requests por minuto
authMiddleware.authenticateWebhook(authConfig.webhook.secretKey), async (req, res) => {
    try {
        ...
        res.status(500).json((0, utils_1.createErrorResponse)('CALLBACK_ERROR', 'Erro interno ao processar callback', process.env.NODE_ENV === 'development' ? { error: error instanceof Error ? error.message : 'Unknown error' } : undefined, req.requestId));
    }
});
/**
 ...
 */
app.post('/callback/document', webhookValidation.validate(), webhookValidation.rateLimit(200, 60000), // 200 requests por minuto
authMiddleware.authenticateWebhook(authConfig.webhook.secretKey), async (req, res) => {
    try {
        // Validar payload básico
        const { document_id, status, processing_info } = req.body;
        if (!document_id || !status) {
            return res.status(400).json((0, utils_1.createErrorResponse)('INVALID_PAYLOAD', 'document_id e status são obrigatórios', undefined, req.requestId));
        }
        // Log do evento
        ...
        res.status(200).json((0, utils_1.createSuccessResponse)({ message: 'Callback de documento processado com sucesso' }, req.requestId));
    }
    catch (error) {
        console.error('Erro ao processar callback de documento:', error);
        res.status(500).json((0, utils_1.createErrorResponse)('CALLBACK_ERROR', 'Erro interno ao processar callback de documento', process.env.NODE_ENV === 'development' ? { error: error instanceof Error ? error.message : 'Unknown error' } : undefined, req.requestId));
    }
});
/**
 * GET /callback/health
...
app.get('/callback/health', (req, res) => {
    try {
        ...
        res.status(200).json((0, utils_1.createSuccessResponse)({
            status: 'healthy',
            service: 'callbacks',
        ...
        res.status(500).json((0, utils_1.createErrorResponse)('HEALTH_CHECK_ERROR', 'Erro ao verificar saúde do serviço de callbacks', undefined, req.requestId));
    }
});
/**
...
app.get('/callback/metrics', (req, res) => {
    try {
        ...
        res.status(500).json((0, utils_1.createErrorResponse)('METRICS_ERROR', 'Erro ao obter métricas de callbacks', undefined, req.requestId));
    }
});
/**
 ...
 */
app.get('/callback/security/stats', (0, authMiddleware_1.requireAuth)(authService), async (req, res) => {
    try {
        ...
        res.status(500).json((0, utils_1.createErrorResponse)('STATS_ERROR', 'Erro ao obter estatísticas de segurança', process.env.NODE_ENV === 'development' ? { error: error instanceof Error ? error.message : 'Unknown error' } : undefined, req.requestId));
    }
});
/**
 ...
 */
app.post('/callback/webhook/test', (0, authMiddleware_1.requireAuth)(authService), webhookValidation.validateSignatureOnly(), async (req, res) => {
    try {
        res.json((0, utils_1.createSuccessResponse)({
        ...
        res.status(500).json((0, utils_1.createErrorResponse)('WEBHOOK_TEST_ERROR', 'Erro ao testar webhook', process.env.NODE_ENV === 'development' ? { error: error instanceof Error ? error.message : 'Unknown error' } : undefined, req.requestId));
    }
});
/**
 * GET /callback/test
...
if (process.env.NODE_ENV === 'development') {
    app.post('/callback/test', (req, res) => {
        try {
            ...
            res.status(500).json((0, utils_1.createErrorResponse)('TEST_ERROR', 'Erro no callback de teste', { error: error instanceof Error ? error.message : 'Unknown error' }, req.requestId));
        }
    });
}
// Middleware de tratamento de rotas não encontradas
app.use('*', (req, res) => {
    res.status(404).json((0, utils_1.createErrorResponse)('ROUTE_NOT_FOUND', `Endpoint de callback não encontrado: ${req.method} ${req.originalUrl}`, undefined, req.requestId));
});
...
exports.callbacksApi = functions
    .region('us-central1')
    .runWith({
    memory: '512MB',
    timeoutSeconds: 60,
