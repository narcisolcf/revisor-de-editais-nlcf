...
class AlertService {
    ...
    async createAlertRule(request) {
        try {
            ...
            this.logger.error('Erro ao criar regra de alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'createAlertRule',
    ...
    async updateAlertRule(ruleId, updates) {
        try {
            ...
            this.logger.error('Erro ao atualizar regra de alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'updateAlertRule',
                metadata: { ruleId }
            });
            throw error;
    ...
    async deleteAlertRule(ruleId) {
        try {
            ...
            this.logger.error('Erro ao remover regra de alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'deleteAlertRule',
                metadata: { ruleId }
            });
            throw error;
    ...
    async getAlertRules(enabled) {
        try {
            ...
            this.logger.error('Erro ao buscar regras de alerta', error instanceof Error ? error : new Error(String(error)), {
    ...
    async evaluateAlerts() {
        ...
        try {
            ...
            this.logger.error('Erro ao avaliar alertas', error instanceof Error ? error : new Error(String(error)), {
    ...
    async evaluateRule(rule) {
        try {
            ...
            const alert = {
                id: this.generateId(),
                ruleId: rule.id,
                status: monitoring_types_1.AlertStatus.ACTIVE,
            ...
            this.logger.error('Erro ao avaliar regra de alerta', error instanceof Error ? error : new Error(String(error)), {
    ...
    async saveAlert(alert) {
        try {
            ...
            this.logger.error('Erro ao salvar alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'saveAlert',
    ...
    async acknowledgeAlert(alertId, acknowledgedBy) {
        try {
            await this.db.collection('alerts').doc(alertId).update({
                status: monitoring_types_1.AlertStatus.ACKNOWLEDGED,
            ...
            this.logger.error('Erro ao reconhecer alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'acknowledgeAlert',
                metadata: { alertId, acknowledgedBy }
            });
            throw error;
    ...
    async resolveAlert(alertId) {
        try {
            await this.db.collection('alerts').doc(alertId).update({
                status: monitoring_types_1.AlertStatus.RESOLVED,
                resolvedAt: new Date(),
                updatedAt: new Date()
            });
            ...
            this.logger.error('Erro ao resolver alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'resolveAlert',
                metadata: { alertId }
            });
            throw error;
    ...
    async getAlerts(query) {
        try {
            ...
            this.logger.error('Erro ao buscar alertas', error instanceof Error ? error : new Error(String(error)), {
                function: 'getAlerts',
                metadata: { query }
            });
            throw error;
    ...
    async sendNotifications(alert, rule) {
        try {
            ...
            this.logger.error('Erro ao enviar notificações', error instanceof Error ? error : new Error(String(error)), {
    ...
    async sendNotification(alert, channel) {
        try {
            ...
            this.logger.error('Erro ao enviar notificação', error instanceof Error ? error : new Error(String(error)), {
                function: 'sendNotification',
                metadata: { alertId: alert.id, channelId: channel.id }
            });
    ...
    async loadAlertRules() {
        try {
            ...
            this.logger.error('Erro ao carregar regras de alerta', error instanceof Error ? error : new Error(String(error)), {
                function: 'loadAlertRules'
            });
        }
    }
    ...
    async loadNotificationChannels() {
        try {
            ...
            this.logger.error('Erro ao carregar canais de notificação', error instanceof Error ? error : new Error(String(error)), {
                function: 'loadNotificationChannels'
            });
        }
    }
    ...
    async getAlertStats(hours = 24) {
        try {
            ...
            const stats = {
                ...
                acknowledged: 0,
                bySeverity: {
                    [monitoring_types_1.AlertSeverity.LOW]: 0,
                    [monitoring_types_1.AlertSeverity.MEDIUM]: 0,
                    [monitoring_types_1.AlertSeverity.HIGH]: 0,
                    [monitoring_types_1.AlertSeverity.CRITICAL]: 0
                }
            };
            snapshot.docs.forEach(doc => {
                ...
                stats.total++;
                switch (alert.status) {
                    case monitoring_types_1.AlertStatus.ACTIVE:
                        stats.active++;
                        break;
                    case monitoring_types_1.AlertStatus.RESOLVED:
                        stats.resolved++;
                        break;
                    case monitoring_types_1.AlertStatus.ACKNOWLEDGED:
                        stats.acknowledged++;
                        break;
                }
            ...
            this.logger.error('Erro ao obter estatísticas de alertas', error instanceof Error ? error : new Error(String(error)), {
