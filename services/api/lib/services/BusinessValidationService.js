...
const FeedbackRepository_1 = require("../db/repositories/FeedbackRepository");
...
class BusinessValidationService {
    constructor(projectId = 'default-project') {
        ...
        this.feedbackRepository = new FeedbackRepository_1.FeedbackRepository(db, 'feedbacks', feedback_types_2.FeedbackSchema);
     ...
     */
    async createFeedback(userId, organizationId, feedbackData) {
        try {
            ...
            const feedbackId = await this.feedbackRepository.createFeedback({
                ...
                status: feedback_types_1.FeedbackStatus.PENDING
            });
            // Notificar equipe se for feedback crítico
            if (feedbackData.rating <= feedback_types_1.SatisfactionRating.DISSATISFIED) {
    ...
    async updateFeedbackStatus(feedbackId, status, reviewedBy, response) {
        ...
        if (response && status === feedback_types_1.FeedbackStatus.RESOLVED) {
     ...
     */
    async trackParameterUsage(userId, organizationId, analysisId, parameterId, parameterName, parameterType) {
     ...
     */
    async analyzeParameterValue(organizationId, days = 30) {
        try {
            ...
            const parameterFeedbacks = await this.feedbackRepository.getFeedbacks({
                organizationId,
                type: feedback_types_1.FeedbackType.PARAMETER_USEFULNESS,
            ...
            for (const [parameterId, usages] of parameterMap) {
                ...
                const relatedFeedbacks = parameterFeedbacks.filter(f => f.metadata?.parameterId === parameterId);
     ...
     */
    async generateBusinessValidationReport(organizationId, startDate, endDate) {
        try {
            // Analisar valor dos parâmetros personalizados
            const parameterAnalysis = await this.analyzeParameterValue(organizationId, Math.ceil((endDate.getTime() - startDate.getTime()) / (24 * 60 * 60 * 1000)));
            // Calcular métricas de satisfação
            const satisfactionMetrics = await this.feedbackRepository.calculateSatisfactionMetrics(organizationId, 'monthly', endDate);
     ...
     */
    async getBusinessMetrics(organizationId, days = 30) {
        try {
            const endDate = new Date();
            const startDate = new Date(endDate.getTime() - (days * 24 * 60 * 60 * 1000));
            // Calcular métricas de satisfação
            const satisfaction = await this.feedbackRepository.calculateSatisfactionMetrics(organizationId, 'monthly', endDate);
            ...
            const validationReport = await this.generateBusinessValidationReport(organizationId, startDate, endDate);
            // Calcular tendências
    ...
    async notifyLowSatisfactionFeedback(feedbackId, feedback) {
        try {
    ...
    async notifyFeedbackResponse(feedbackId, response) {
    ...
    calculateWeeklyTrend(usages, days) {
        // Implementação simplificada - calcular uso por semana
        const weeks = Math.ceil(days / 7);
        const weeklyData = new Array(weeks).fill(0);
        usages.forEach(usage => {
            const weekIndex = Math.floor((new Date().getTime() - usage.lastUsed.getTime()) / (7 * 24 * 60 * 60 * 1000));
    ...
    calculateMonthlyTrend(usages, days) {
        // Implementação simplificada - calcular uso por mês
        const months = Math.ceil(days / 30);
        const monthlyData = new Array(months).fill(0);
        usages.forEach(usage => {
            const monthIndex = Math.floor((new Date().getTime() - usage.lastUsed.getTime()) / (30 * 24 * 60 * 60 * 1000));
    ...
    calculateEffectivenessScore(usage, satisfaction, users) {
    ...
    validateCustomParametersHypothesis(analysis) {
        const evidence = [];
    ...
    async analyzeSatisfactionTrend(organizationId, startDate, endDate) {
    ...
    async calculateFeatureAdoption(organizationId, startDate, endDate) {
    ...
    generateRecommendations(parameterAnalysis, satisfactionMetrics, featureAdoption) {
    ...
    generateNextActions(parameterAnalysis, satisfactionMetrics) {
        const actions = [];
    ...
    async calculateTrends(organizationId, days) {
...
//# sourceMappingURL=BusinessValidationService.js.map